define(['durandal/app', 'api/datacontext', 'game/canvas/vm/Box', 'paper'], function (app, ctx, Box) {
    ...
    Path.prototype.put = function () {
        if (this._canPut && activeWords() != null && !ctx.gameOver()) {
            this._canPut = false;
    ...
    Path.prototype.show = function () {
        ...
        for (var i = 0; i < nWords; i++) {
            ...
            var point = path.getPointAt(offset),
              ...
              hoverArea = this.createHoverArea(path, offset, box.scaledWidth() + 2 * Path.options.rectMargin + (delta > 0 ? delta / nWords : 0));
            hoverArea.data = box;
            offset += half;
